...
//		statusController.coY.textProperty().set(evt.getY()+"");
	...
	public boolean onQuitter() {
		if (!this.dessin.estModifieProperty().get()) {
			...
			boolean confirmation = iut.gon.gribouille_tp1.Dialogue.confirmation();
			if (confirmation) {
				try {					
	...
	public boolean sauvegarder() {
		File fichier = iut.gon.gribouille_tp1.Dialogue.sauverFichier();
		if (fichier != null) {
	...
	public boolean charge() {
		File fichier = iut.gon.gribouille_tp1.Dialogue.chargerFichier();
		if (fichier != null) {			
			this.dessin.setNomDuFichier(fichier.getName());
